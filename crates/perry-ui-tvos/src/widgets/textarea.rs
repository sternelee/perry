...
pub fn create(placeholder_ptr: *const u8, on_change: f64) -> i64 {
    ...
    unsafe {
        ...
        // Set up the delegate for text change notifications
        ...
        let tv_raw = Retained::as_ptr(&text_view) as *const AnyObject;
        TEXTAREA_CALLBACKS.with(|cbs| {
            cbs.borrow_mut().insert(delegate_addr, (on_change, tv_raw));
        });
...
/// Get the current text content from a UITextView, returns a StringHeader pointer.
